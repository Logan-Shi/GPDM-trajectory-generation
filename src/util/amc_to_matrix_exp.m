...
locations = [1 7:3:3*30];
ryz = [8 15];
rxz = [12 14 19 21 24 28];
rx = [10 13 17 20 23 25 27 29];
ry = [11 18];
 ...
 if len == 6
     ...
     if len == 3
         ...
         if len == 2
             ...
             if (ismember(index,ryz))
                 x = Euler_to_Exp([0 xt(1) xt(2)]);
             elseif (ismember(index,rxz))
                 x = Euler_to_Exp([xt(1) 0 xt(2)]);
             end
         else 
             if len == 1
                 xt = fscanf (fid,'%f\n',1);
                 if (ismember(index,rx))
                     x = Euler_to_Exp([xt 0 0]);
                 elseif (ismember(index,ry))
                     x = Euler_to_Exp([0 xt 0]);
                 end
